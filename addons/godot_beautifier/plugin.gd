...
func _enter_tree() -> void:
	...
	if !DirAccess.dir_exists_absolute(get_editor_data_folder()):
	...
	if ProjectSettings.has_setting(_SettingsPanel.P_EnableRandomTheme) && ProjectSettings.get_setting(_SettingsPanel.P_EnableRandomTheme):
		...
		script_path = get_current_dir().path_join("themes/Nana7mi/theme.gd")
...
func get_global_data_path() -> String:
	return get_editor_data_folder().path_join("beautifier-2.cfg")
...
func _on_menu_id_pressed(p_id : int) -> void:
	match p_id:
		MenuID.SETTINGS:
			_SettingsPanel.popup_centered_ratio(0.5)
...
func clear_global_data() -> void:
	if FileAccess.file_exists(get_global_data_path()):
		DirAccess.remove_absolute(get_global_data_path())
